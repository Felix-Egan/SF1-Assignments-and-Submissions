...
def skip_card(current_player):
    print(f"Player {current_player + 2} skips their turn")
...
def change_color(face_up):
    ...
    if colors[chosen_index] == face_up_pile[1]:
...
def play_call_UNO(player_hand, face_up, draw_pile, current_player):
    face_up = play_round(player_hand, face_up, draw_pile, current_player)
...
def play_round(player_hand, face_up, draw_pile, current_player):
    ...
    if not valid_cards:
        ...
        draw_card(player_hand, draw_pile, face_up, current_player)
    else:
...
def main_loop(player_hands, face_up, draw_pile, face_up_pile, num_players, current_player):
    while all(player_hands):
        ...
        if play: play = int(play)
            ...
            main_loop(player_hands, face_up, draw_pile, face_up_pile, num_players, current_player)
        if 0 > play <3: # ooh look a heart ❤️
            print("Please enter a valid index...")
            main_loop(player_hands, face_up, draw_pile, face_up_pile, num_players, current_player)
        
        if   play == 0: draw_card(player_hands[current_player], face_up, draw_pile, current_player)
        elif play == 1: face_up = play_round(player_hands[current_player], face_up, draw_pile, current_player)
        elif play == 2: 
            if len(player_hands[current_player]) == 2 and [card for card in player_hands[current_player] if valid_play(card, face_up)]: face_up = play_call_UNO(player_hands[current_player], face_up, draw_pile)
            else:
